...
knitr::opts_chunk$set(echo = TRUE, options(dplyr.print_max = 5))
library(dplyr)
library(lubridate)
library(magrittr)
library(ggplot2)
...
## Special Note for Exercise Sheet 1 (1 Point)

As we are focusing on Rmarkdown itself for this first exercise sheet, we
kindly ask you to submit solutions as both HTML and PDF document.

-   For the HTML document, please use the `distill` package as a
    template.
-   For the PDF document you can use the standard PDF renderer in
    Rmarkdown. You can use the dropdown menu on the `Knit` button to
    manually render to PDF.

Note: If you have some issues with rendering to PDF this is really not a
big problem, it will not cost you many points.
...
```{r}
(car_rentals %<>%
  tidyr::unite("rental_date", rental_date_year, rental_date_month, rental_date_day, sep = "-") %>%
...
```{r}
(car_rentals %<>%
  tidyr::unite("return_date", return_date_year, return_date_month, return_date_day, sep = "-") %>%
...
languages %>%
  ggplot(aes(x = number_of_users, y = book_count)) +
...
users a given language has, the greater the quantity of published books
are available for it; however, there is a great amount of variability in
this. Plot generated using the {ggplot2} package [@wickham2016].
...
(book_user_cor <- cor(languages$number_of_users, languages$book_count))

```

There is a correlation of `r round(book_user_cor, 3)` between the number
of books published and the number of users of a language.
...
languages %>%
  ...
  rename(Language = title, "Number of Users" = number_of_users, "User Fraction" = user_fraction) %>%
  knitr::kable(digits = 3, caption = "Is this needed?")
```
...
Text about R [@rcoreteam2024] which is in the Table
\@ref(tab:popular-langs)
...
the years immediately preceding the second world war given that many advancements
towards modern computing began around this time.
